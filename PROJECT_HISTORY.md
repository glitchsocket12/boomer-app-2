...
## 2026-09-04 — Why saving on Home took 7-17 seconds, and what the logs actually said

**The report:** "It takes the home page a LONG time to process a piece of information into the grove database. Why is it taking so long, and can we improve the processing time?"

**The measurement, before touching anything.** Home has exactly one capture path — the chat box → `converse` — so there was one function to look at. `function_edge_logs` over three days: `converse` averaged ~6.9s (p50 5.4-6.8s, max 16.8s), the slowest function in the app by a wide margin; next worst was `update-moment` at ~4.7s. Correlating `execution_time_ms` against the `usage` line in `function_logs` gave the decomposition:

| call | server time | prompt tokens | output (thinking) |
|---|---|---|---|
| 09-04 16:19:14 | 16,833 ms | 99,158 created, 0 read | 1,210 (754) |
| 09-04 16:19:39 | 10,596 ms | 60,915 created, 38,541 read | 754 (357) |
| 09-03 02:31:20 | 13,464 ms | 98,237 created, 0 read | 1,064 (654) |

The database was not the problem and never had been: writes measured 0.15-0.35s, the 12 context reads are already one `Promise.all`, and `summarize-moment` was already fired-and-forgotten through `EdgeRuntime.waitUntil`. Essentially 100% of the wait was the single Anthropic call.

**The finding worth keeping — a random UUID was silently costing ~61,000 tokens a message.** The 1-hour tier (instructions + roster, 38,541 tokens) was reading from cache correctly. The moments tier never was: `cache_creation` on all three sampled calls, never `cache_read`. The cause turned out to be one `.order("id")`. Moment ids are random UUIDs, so id-order has no relationship to time-order — checked against the live database, the rank correlation between the two orderings was **-0.038, with only 2 of 204 moments in the same position under both**. Every newly saved moment therefore landed at a *random point in the middle* of a 61K-token block that carried a single cache breakpoint at its end, destroying the prefix. Compounding it, the tier sat on the default 5-minute TTL, so a pause between messages expired it anyway — and `converse` writes to the very moments it puts in its own cached prefix, so the tier was self-invalidating for exactly the action being complained about.

The fix is ordering and breakpoint placement only: `created_at` ASC (tiebreak `id`) so the block is append-only, split into a 1h-cached archive plus the newest 20 held out uncached, notebooks moved up into the roster tier, and the 4th breakpoint moved off the message thread onto the archive. **Nothing was dropped or shortened** — worth stating plainly, because it sits next to the archive-size trade the founder declined on 2026-07-20 and is not that trade.

**The founder's call on the second lever.** ~60% of output tokens were thinking (754 of 1,210; 654 of 1,064; 357 of 754) — `converse` passes no `output_config`, so Sonnet 5 runs adaptive thinking at its `high` default by omission. Offered three options; they chose **"free fixes, then measure before trading"** — ship what costs nothing, and decide the effort dial against real numbers rather than an estimate. Consistent with the standing "free fixes only, no trade-offs" position. A `timing` log line (`db_ms` / `ttft_ms` / `stream_ms` / `write_ms` / `total_ms`) was added for exactly that decision, and streaming is what makes `ttft_ms` — time to the first *word*, i.e. the thinking time — measurable at all.

**What streaming cost structurally.** The response has to be on the wire before the model finishes, so the entire write pass now runs *inside* the stream: ~680 lines moved into a `streamTask` IIFE, dispatched with `EdgeRuntime.waitUntil`, with failures after the first byte travelling as an `error` event because the status code is long gone by then. Two consequences that needed deciding rather than discovering later — `send()` deliberately swallows write failures (if the user closes the tab mid-reply, the writer rejects, and letting that propagate would abort the write pass and lose the note they just dictated), and the client's version-skew branch is load-bearing rather than defensive, because a `git push` redeploys only Vercel while the Edge Function is a separate deploy.

**Testing, given no credentials.** The `.env` `SUPABASE_ACCESS_TOKEN` had been revoked (401 on both the CLI and the Management API), and the MCP `deploy_edge_function` route was impractical — `converse`'s transitive import closure is 329KB across 18 files, including a 116KB generated name-gender table. So the streaming path was proved without deploying: 20 unit tests over `createReplyExtractor`/`readAnthropicSse` (feeding envelopes one character at a time and chopping SSE bodies every 5-7 bytes, since a chunk boundary can land mid-`\uXXXX`), plus the real `converseStream` module driven in the browser against a stubbed `fetch` — streamed turn, version-skew JSON turn, and mid-stream error all confirmed, the last one preserving the partial text the user had already read. One of those tests caught a genuine bug: the extractor's give-up cap never latched, because trimming the seek buffer kept resetting its length below the cap it was being compared against.

**The lesson: a cache tier that reports `cache_creation` every single time is not "cold", it is broken, and the cause can be a single `.order()` clause three lines from the query.** The tiering had been built correctly and reviewed as correct on 2026-07-20; what was never checked was whether the *ordering feeding it* was stable under the writes the same function performs. `usage.cache_read_input_tokens` being non-zero on a repeat call — the check CLAUDE.md rule 3 already mandates — would only have caught this if run after a turn that actually wrote something, which is the case that matters and the case least likely to be tested.

### 2026-09-04, same day — two corrections found only by deploying and measuring

Both of these were caught because the new `timing` log has a `ttft_ms` field that reads `null` when the reply never streamed. Without that one field, both would have shipped looking fine: the UI showed a correct answer in every case, just not a streamed one.

**1. `thinking: { display: "summarized" }` broke the JSON envelope.** It was added on documented grounds — display is free, thinking happens and bills the same either way — to give the UI a live status line during the wait. On the two live calls after deploying, the model stopped emitting the JSON envelope this function is built on and answered in plain prose instead. Both fell through to the pre-existing prose salvage path, so the reply read perfectly while *every write field was silently discarded* — notes, moments, family signals, all gone, with a cheerful confirmation. One of the two also spent 6,801 thinking tokens and 59.7 seconds, against 357-754 tokens on comparable calls before the change. Removed; the parameter is now absent with a comment saying why, because omitting it runs adaptive thinking anyway and that is what the function was tuned against. **The general lesson: a request parameter documented as not affecting output can still change how well the model follows the prompt, and in a function whose reply is a side channel around a JSON contract, "the answer still looks right" is not evidence the turn worked.**

**2. The real reason lookups didn't stream — and it was not the thinking parameter.** After removing `thinking`, `ttft_ms` was *still* null. The first hypothesis was wrong. The actual cause was sitting in a comment already in the file: "the model sometimes just answers in plain prose despite the instruction" — converse has had a prose salvage path since long before streaming. All three test questions ("who is in X", "how many people in X", "which group is X in") were pure lookups, which is exactly when the model skips the envelope, and the extractor could only stream text found inside a `"reply"` field. So `createReplyExtractor` now decides envelope-vs-prose from the first non-whitespace character — `{` or a ``` fence means envelope, anything else means stream the text verbatim. Confirmed live afterwards: `ttft_ms: 4702`, then `3006` on the next turn.

**Worth keeping: three test cases in a row that all avoided the write path is not a sample, it's a blind spot.** Lookups were chosen precisely because they were safe to run against the founder's real account — and safety and representativeness pulled in opposite directions. The founder's own real save at 20:10 (`write_ms: 1076`, envelope parsed cleanly, `ttft_ms: 15803`) is what showed the envelope path had been streaming correctly the whole time.

**The measured result, for the record.** Warm cache, same account, same day: `cache_read_input_tokens: 97,487` with `cache_creation: 0` — the entire prompt served from cache, against a baseline where 60,915 of 99,158 tokens were re-created on every single turn. `timing {"db_ms":1614,"ttft_ms":3006,"stream_ms":3921,"write_ms":0,"total_ms":5535}` — 5.5s end to end with the first words on screen at ~4.6s, against a 6.9s average (max 16.8s) that showed nothing at all until it was completely finished. And the split the founder deferred their effort decision on is now visible: on a real save with heavy reasoning, thinking was 15.8s of a 20.4s turn.

**Deploy footnote, cost several turns.** The `.env` token was revoked mid-session and the replacement came back in the new `sbp_v0_...` format, which **both the pinned and the latest Supabase CLI reject client-side** as "Invalid access token format" — the token itself is fine (200 against the Management API). The working path is a direct multipart `POST /v1/projects/<ref>/functions/deploy?slug=<slug>` with one `-F file=@<path>;filename=<path>` per file in the transitive import closure (18 files for converse) plus a `metadata` part. Two traps: the Bash tool's version of that curl gets classifier-blocked (PowerShell's `curl.exe` goes through), and PowerShell strips the quotes out of an inline JSON `-F metadata={...}` — write the metadata to a file and use `-F "metadata=<file;type=application/json"`. Also `$args` is a reserved PowerShell automatic variable, and splatting it silently sends nothing.

### 2026-09-04, later — effort "medium" was fast, and silently lost a save

The founder took the Phase 2 decision with the measured split in front of them ("let's try changing it to medium") and it was reverted within the hour, on evidence.

**What it bought.** `output_config: { effort: "medium" }`, deployed as v87. Thinking collapsed from 138-754 tokens to 0-30. Same two questions, warm cache, before → after: "Where did I meet Manuel Sucre?" 7,658ms → 5,671ms; "What was the last event I recorded?" 5,535ms → 3,349ms. Answers stayed substantive and correct, including the relationship reasoning ("married to Clare Sucre (Caroline's sister), making him your brother-in-law"). On speed and on the quality of *answers*, medium won clearly.

**What it cost.** Two test captures were run against the founder's real account. The first wrote correctly (`write_ms: 229`, envelope parsed, `thinking_tokens: 30`). The second replied **"Noted — logged as a moment on September 4, 2026 at home, just you."** and wrote nothing at all — `write_ms: 1`, `output_tokens: 27`, `thinking_tokens: 0`. Checked against the database directly: the moment did not exist. Reverted to the "high" default as v89.

The mechanism was already in the file: converse has carried a plain-prose salvage path for months because the model sometimes answers conversationally instead of emitting the JSON envelope. A turn that doesn't think is markedly more likely to do that, and without the envelope every write field — notes, moments, groups, family signals — is discarded while the reply still reads perfectly. **On a memory archive, "told the user it saved and didn't" is the worst failure available, and no amount of speed buys it back.** The comment in the file now says so, with the numbers, so the next person to look at the effort dial doesn't re-run the experiment blind.

**The wider point this exposed, which outlives the effort question:** this failure mode exists at "high" too — it is just rarer. converse can accept a capture, drop it, and confirm it. Nothing in the UI distinguishes that from success, and until this session nothing in the logs did either. Worth treating as its own bug rather than a property of the effort setting.

### The second half of the caching fix, found the same way

Deploying the first version and then *measuring the turn after a write* showed the archive tier was still being re-created — `cache_creation: 57,528` on the message following a capture, exactly the case the split existed to protect.

The cut was taken at a fixed distance from the end (`length - 20`). Appending a moment grows the list, which slides that boundary forward, which pushes the oldest held-out moment **onto the end of the archive** — and a block one line longer no longer matches the cached prefix that terminates at its breakpoint. So the archive was stable against everything except the one action the whole change existed to make cheap.
...
## 2026-09-04 — The Calendar's "Today" button, and why smooth scrolling can't be trusted

The Timeline's "Today" button did nothing. The on-load centring worked; the button, which ran the same function with `behavior: 'smooth'` instead of `'auto'`, produced no movement at all. Measured in the page, `marker.scrollIntoView({ block: 'center' })` moved the list 0 → 11,150 and the identical call with `behavior: 'smooth'` left it at 0 after three seconds — app code entirely bypassed, so nothing to do with React, refs or state.

**The cause is that a browser only animates a scroll while it is actually painting the page.** When it isn't, the animation isn't queued for later — it is dropped, silently, and `scrollTop` never moves. That covers a background tab, a minimised window, and a window sitting behind another one. Crucially the page still reports `document.visibilityState === 'visible'` in the last case, so there is no flag to test and no error to see; `requestAnimationFrame` callbacks and even `setTimeout` are frozen in the same state.

**This is also a measurement trap, and it wasted most of the session.** Every environment available for testing this — the desktop app's preview pane, a Chrome tab driven over MCP — is by definition a window nobody is looking at, so *any* smooth-scroll test reads as a failure there whether or not the code is right. Two separate hours went into "the fix didn't work" readings that were really "the pane wasn't drawing". The tell is that scripted `btn.click()` worked when the pane was fully hidden (the code took an instant-jump branch) and stopped working once the pane was merely visible-but-not-painting (it took the animated branch and froze). If a future session needs to measure animation in this repo, measure the landing position across two separate tool calls, never inside one long-polling script.

The fix is `src/lib/centerInScroller.ts`: compute the target from `getBoundingClientRect()` against the scroll container, animate `scrollTop` by hand over 420ms, and **back the animation with a `setTimeout` that puts the box on the target regardless**. Worst case the list arrives without the glide, which is still the right place — the button can never again be silently dead. Two details worth keeping: `offsetTop` was not usable here because the Timeline's list isn't a positioned ancestor of the Today divider (its `offsetParent` is `<body>`, giving an answer wrong by the height of the whole page above the list), and `scrollIntoView` was wrong independently of the animation because it scrolls every scrollable ancestor and so drags the page — the same jumping-around the founder had already asked to be rid of on Countdowns months earlier.

Countdowns' own Today button was moved onto the same helper rather than leaving two implementations of one idea. It had the identical smooth-scroll exposure and its comment claiming `position: relative` was load-bearing for scroll maths is no longer true.
