...
export default function EventDetail({
  ...
  async function loadMoment() {
    ...
    const { data } = await supabase
      .from('moments')
      .select('id, occasion, location, when_text, raw_description, details, created_at, notes(id, content, created_at, people(id, name, last_name))')
      .eq('id', eventId)
      .single()
...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  meta: { margin: '0 0 1rem 0', fontSize: '0.95rem', color: '#888' },
  ...
  subheading: { fontSize: '1.2rem', color: '#2E4034', margin: '1.5rem 0 0.75rem 0' },
