...
async function scanUser(supabase: SupabaseClient, userId: string, preview: boolean): Promise<ScanResult> {
  ...
  const results = await Promise.all(
    chunks.map(async (chunk) => {
      ...
      // Cast through `unknown`, as Events.tsx and EventDetail.tsx do for the identical embed:
      // moment_tags → tags is many-to-ONE, so PostgREST returns an object, but with no generated
      // database types the client infers an array and the two shapes don't overlap.
      for (
        const row of (tagRowsRes.data ?? []) as unknown as { moment_id: string; tags: { name: string } | null }[]
      ) {
