...
serve(async (req) => {
  ...
  try {
    ...
    const stableInstructions = `You are helping the user add more detail to a memory they already recorded in an app called Grove.
...
CRITICAL — never invent, assume, or add a concrete detail the user did not actually say, in "additional_notes" OR "reply". If they don't state HOW something happened, a result, or how someone felt, don't supply a plausible-sounding guess for it — leave it out entirely, even if it feels like an obvious or typical part of the situation. And an "additional_notes" entry only belongs to a specific attendee when THEY are the one who did, said, or experienced the thing described — not merely because the sentence names them or is about them (e.g. "we found out the baby's going to be a girl" is a fact about the event, never a note on the baby's own profile, even though the baby is named — the baby didn't discover or announce anything).
