...
serve(async (req) => {
  ...
  try {
    ...
    const stableInstructions = `You are helping the user edit a group in an app called Grove. Groups tag together people and events that share a recurring affiliation (a team, a school, a workplace, a family branch, etc.). A group's membership (who belongs to the group) is intentionally independent from which events are tagged to it — someone can be a member without having attended every, or any, tagged event.
