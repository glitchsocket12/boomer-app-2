...
- **Writes are scoped too** (second pass, same day): every write condition either names `auth.uid()` directly or inherits the read rule, which is what Postgres does automatically for `FOR ALL` and `FOR UPDATE` policies. A blank write condition on those is normal and safe, not a hole.
...
0. **Nothing outstanding — writes were verified clean too (2026-08-01, second pass).** Every write rule in the database is scoped to the owner. Two things worth recording, because both were flagged as concerns on the first pass and both turned out to be fine: `group_associations`' *read* rule only checks `group_id_a`, but its *write* rule requires **both** groups belong to you, so the cross-account row that asymmetry seemed to allow can never be created. And the four INSERT policies that looked blank (`home_suggestions`, `notes_group_insert`, `photo_connections`, `relationships_insert_own`) are all correctly scoped — they only looked blank because INSERT rules carry a write condition rather than a read one. One structural note for the future: `notes` carries six overlapping rules because a note can hang off a person, a moment, or a group. Correct today, but it's the most intricate rule set in the database — **if a fourth kind of note is ever added, re-run the audit and re-read that group specifically.**
