...
user_settings user_id (PK), chat_tone (text, CHECK-constrained to 'warm'/'direct'/
              ...
              **Migration not yet applied to the live DB as of 2026-07-24 — see §10.**
...
- **Founder needs to run a SQL migration + redeploy 3 Edge Functions: time zone bug fix (2026-07-24, item — "today" mis-dating evening events)** — `supabase/migrations_manual/2026-07-24-user-settings-timezone.sql` (adds `user_settings.time_zone`) needs pasting into the Supabase SQL Editor, then `converse`/`update-moment`/`scan-calendar-sources` need redeploying (`npx supabase functions deploy <name>` with a fresh Personal Access Token, or dashboard paste) so the shared `_shared/tz.ts`/updated `_shared/userSettings.ts`/`_shared/ics.ts` bundle in. Until both land, the Settings time-zone picker's save silently no-ops (upsert 400s on the missing column, verified live 2026-07-24 via a real browser session against `jakevolin@gmail.com` — no data was actually written) and the deployed Edge Functions keep computing "today" from server UTC.
