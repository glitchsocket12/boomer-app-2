...
│   │                            **'pending'** (2026-08-22, was 'selected'), called on load by the
│   │                            inbox and the queue (no cron) — waking into the COUNTED queue would
│   │                            mean "set aside all 1,300" hands Home "1,300 ready to review" six
│   │                            months later. Costs one tap on the rarer path (a card set aside
│   │                            from ImportReview returns to triage, whose row leads with "Add More
│   │                            Detail"); telling the two origins apart needs a column and a
│   │                            hand-run migration. ReviewInbox's "Show them now" matches.
│   │                            `canBulkSetAside()` + `BULK_SET_ASIDE_MIN` (25) gate the bulk
│   │                            control — own tested predicate because it guards the largest single
│   │                            write in the app: never pre-migration, never beside a search or the
│   │                            turned-down list, never on a pile small enough to just do.
│   │                            `canBulkSetAsideRoutine()` is its own predicate, not an argument on
│   │                            the first: different set, different safety story, and it is never
│   │                            offered until every pending row is scored — acting on a partial
│   │                            classification would set events aside purely because the backfill
│   │                            hadn't reached them. `probeSignificanceEnabled()` mirrors the
│   │                            triage probe for `2026-08-27-candidate-significance.sql`.
...
│   │                            **"Set aside the rest" (2026-08-22, item 104's escape hatch):** a
│   │                            bar above the list defers EVERY pending row at once via RemindSheet
│   │                            (no default applied — at this size the interval is the decision),
│   │                            written server-side by `.eq('status','pending')`, never a 1,300-id
│   │                            `.in()` (~48KB URL). Stamps one `reviewed_at` across the act so the
│   │                            UndoBanner can take back exactly those rows. Gated by
│   │                            `canBulkSetAside()`. Confirmations from the sitting stay on screen;
│   │                            the rest clear. `writeError` is a message string, not a boolean —
│   │                            "Couldn't save that one / a migration is missing" is a lie after a
│   │                            bulk press, and the bulk copy never claims nothing was written.
│   │                            **Recommended (2026-08-27):** `showTurnedDown` became
│   │                            `view: 'recommended' | 'all' | 'turnedDown'` behind a chip row —
│   │                            a lens on the SAME pending pile, not a second list, so pagination,
│   │                            search, the four-answer rows, resolved/Undo and both bulk controls
│   │                            are shared. Lands on Recommended once one exists (founder choice);
│   │                            a line under the chips says these are guesses, not a filter, and
│   │                            names the full count. Rows show their kind ("looks like a trip").
│   │                            "Find the ones worth keeping" loops `score-candidates` with live
│   │                            progress, a Stop, and MAX_SCORING_CALLS — it spends money, so it
│   │                            can't run away. Second bulk button sets aside only
│   │                            `significance='routine'`; it SUPERSEDES the blanket one rather than
│   │                            stacking beside it. Column list is probe-dependent: selecting an
│   │                            unknown column is a hard error, so the first fetch waits on `ready`.
...
candidates    id, user_id, calendar_source_id, ical_uid (unique per user, dedupes
              ...
              **Two values added 2026-08-19**
              ...
              which is where an untriaged candidate belongs), significance?
              (2026-08-27, `migrations_manual/2026-08-27-candidate-significance.sql`:
              trip/celebration/milestone/holiday/gathering/routine, written by
              the extraction call or the score-candidates backfill; null = not
              scored yet. Anything but 'routine' shows in triage's Recommended
              list. NEVER used to hide or reject — every candidate stays in the
              full list, the 2026-08-12 directive is untouched), occasion?,
...
104. **Triage relief** — founder: mass bulk import "sets a general shell, but it's an overload, very hard to work through," and "too much to triage" is one of three stated reasons for not opening the app. Three parts: auto-accept only what's certain (per §9's act-only-when-certain boundary, logged via 103), stop showing a running count of how far behind you are, and a one-action "archive the whole queue, recoverable" escape hatch. Note this partly revisits the 2026-08-19 decision NOT to build a bulk accept-all — revisited deliberately, after living with the pile. Does **not** reverse the 2026-08-12 "sync everything, let the person decide" directive: nothing here filters. **Two of the three DONE 2026-08-22**, after the founder's 1,300-event sync ("still very overwhelming"): the running count is gone from Home (`reviewTotal` drops both untriaged piles), and the escape hatch is `CalendarTriage`'s "Set aside the rest" — every pending row deferred in one act, recoverable from its own UndoBanner or the inbox's "Show them now", and returning to the QUIET pile rather than the counted one. **Recommended DONE 2026-08-27** (needs `migrations_manual/2026-08-27-candidate-significance.sql`, §10): the extraction call that already reads every event now also returns a `significance` KIND (trip / celebration / milestone / holiday / gathering / routine) — no new call, no new input tokens, a couple of output tokens per event. A new `score-candidates` function backfills the 1,300 already synced (~$0.25 one-off) behind a button with live progress and a Stop. Triage gained a Recommended chip that it lands on, rows that say why ("looks like a trip"), and a second bulk button that sets aside only the routine ones. Founder decisions: yes to that button, and yes to landing on Recommended. **Rank, never hide** — every event stays one tap away under "Everything", so this does not reopen the 2026-08-12 filter. **Still open:** auto-accept-what's-certain (the third part).
...
- **Founder action needed: run `migrations_manual/2026-08-27-candidate-significance.sql` (item 104's Recommended half).** One column + CHECK + partial index on `moment_import_candidates`, safe to re-run. Until it runs, `probeSignificanceEnabled()` is false and the triage page has no Recommended chip, no "Find the ones worth keeping" button and no routine bulk control — it behaves exactly as it does today, and `scan-calendar-sources` leaves the field off every row rather than failing the upsert. Also needs `score-candidates` deploying. **Not verified against the real database**: the Edge Functions could not even be type-checked in that session (the environment's proxy denies `deno.land` and `esm.sh`, so `npm run check:functions` cannot run there — they parse cleanly under esbuild and nothing more), and the UI was driven against a stub, not real rows under RLS.
