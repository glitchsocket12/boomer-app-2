...
- **NEEDS DEPLOY: 5 Edge Functions for item 94's group-name fix (2026-08-11)** — `add-fact`, `converse`, `scan-calendar-sources`, `suggest-prompts`, `update-moment` (everything importing `_shared/groupNames.ts`). Until this ships, the AI still qualifies subgroups one level deep while the app shows the full chain, so a subgroup nested 3+ deep can be tagged to the wrong parent. Deploy with `npx supabase functions deploy <name>` and confirm live with the token-free check (`UNAUTHORIZED_NO_AUTH_HEADER`, not Supabase's `NOT_FOUND`). The same-day type-only fixes to `converse`/`update-group`/`summarize-group`/`transcribe`/`google-photos-oauth-callback` (item 95) are no-ops at runtime and don't need a deploy of their own, but they'll ride along.
