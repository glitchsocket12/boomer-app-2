...
78. **Landing page should call out the calendar-import feature** — reported via feedback widget 2026-07-27, from the Calendar page's "202 events found from your calendar — review" banner. Founder wants front-page copy to explicitly describe this (auto-builds event shells from your calendar, then prompts you over time to fill them in) since it's a strong selling point not currently mentioned pre-signup.
...
- *(Home "Connections to make" card, 2026-07-27)* Founder: "This is also a great feature - we probably need to beef it up." No specifics given yet on what "beefed up" means — needs the founder to define the actual ask before this becomes a scoped item.
...
- **Founder action needed: run `migrations_manual/2026-08-06-countdown-settings.sql` (item 84)** — adds `custom_title` / `units` / `repeat_rule` / `keep_counting` to `countdowns`, for the per-card ⚙ panel. Run `2026-08-06-countdowns.sql` first if it somehow hasn't been; safe to paste twice. Until it runs, `loadCountdowns` asks for the new columns, gets a 42703, retries with the original column list and hides the ⚙ — everything else in the section (including the fixed × and the Today line, which need no new columns) works normally. After running it, worth clicking once: rename a card and confirm the event's own title is untouched, pick "Days" on an old milestone (should read a total in the thousands), set something to repeat and confirm it jumps to the next occurrence, and reload to confirm all of it stuck.
- **Founder action needed: run `migrations_manual/2026-08-06-countdowns.sql` (item 83)** — creates the `countdowns` table the Calendar page's new Countdowns section writes to. **The file is now re-runnable** (2026-08-06 fix): the founder's first paste hit `42710: policy "Users manage their own countdowns" already exists` on a second run, because `create policy` has no IF NOT EXISTS while every other statement in the file does — a `drop policy if exists` now precedes it, same as `2026-07-20-relationships-table.sql`. Worth remembering for any future migration that creates a policy. Until it runs, that section still works read-only: auto-derived milestones (Milestone-tagged past events, birthdays/anniversaries with a year) render normally, but "+ Add" and the dismiss × are hidden rather than offering writes that would silently no-op — verified in a browser, no error, no crash. After running it, the things worth clicking once are the three "+ Add" paths, dismissing a milestone (must survive a reload, and must leave the event itself alone), and deleting a pinned event (its countdown should vanish with it via ON DELETE CASCADE — the one behaviour that couldn't be tested without a real database).
