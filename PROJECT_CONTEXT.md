...
- Person detail page (click into a tile): shows the standardized read-only field cells — Last name, Birthday, Anniversary (plus any other custom-labeled date on file) — each showing its value or a subtle "not on file" placeholder, so it's obvious what info is still missing. These fields are **read-only on purpose**; adding/correcting them happens through the chat (`add-fact` currently only handles last-name corrections and plain notes — see Section 8/limitations, birthday/anniversary reminders aren't yet chat-editable).
- Reminders: the `reminders` table (Birthday/Anniversary/custom labels) is currently populated by manual DB inserts done during this redesign work, not by any in-app add flow — that was intentionally removed from the People page. **No automatic email/notification sending exists yet.**
