...
│   │                            Calendar page's existing data is reused). DB side: `loadCountdowns`
│   │                            (returns `{rows, available}`, same fail-open contract as pets.ts),
│   │                            `addCountdown`, `pinMoment` (un-hides rather than duplicating),
│   │                            `dismissCountdown` (delete / hide / hide-derived, never touches the
│   │                            underlying event or birthday).
...
│   │                            (2026-08-06, item 83) `CountdownsSection` renders ABOVE Upcoming;
│   │                            the people select gained `deceased_date` + `reminders.year` for it.
...
│   ├── CountdownsSection.tsx  — (2026-08-06, item 83) collapsible "Countdowns" card at the top of
│   │                            Calendar.tsx: one card per subject, title + ↑ (how long it's BEEN)
...
countdowns    id, user_id, label?, target_date?, moment_id? (FK moments, ON DELETE
              ...
              computed (upcoming soonest-first, then past most-recent-first).
