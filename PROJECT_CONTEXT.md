...
- **Item 96 click-testing, 2026-08-21 — most of it done, three paths still unseen.** Verified against the founder's real account at merge time, each write reversed afterwards and the queue counts confirmed back at baseline (294 pending / 6 selected / 189 rejected / 176 accepted): one Home nudge reading "1,122 things to review"; all four inbox rows with counts matching the DB; Quick Add creating a real event and Undo deleting it again cleanly; Remind Me applying the saved 30-day default without a sheet, and its Undo; the gender pass saving through the dropdown, dropping the name off the can't-guess list, and the count falling 60 → 59; the contacts queue with its All / Already in Boomer / New people filters; the detailed card showing a possible-duplicate warning. **Still unseen:** a batch of 10 ending with the summary panel, a collapsed **Accept** on the detailed card saving the right title/date/attendees (skipped on purpose — unlike Quick Add it has no one-tap Undo, so testing it means leaving or hand-deleting a real event), and **drag**-into-Men/Women (only the dropdown path was exercised; both call the same setter, but the drag handlers themselves have never run).
