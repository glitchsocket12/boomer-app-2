...
│   │                            scrolling page (What is Porch? / Not another social
│   │                            network incl. a Porch-vs-social-media/journaling-apps/
...
│   │                            Nav "Porch" wordmark is now a button that smooth-scrolls
│   │                            to page top.
...
│   │                            leads the What-is-Porch and How-it-works sections; keep
...
│   │                            clickable "Porch" wordmark (2026-07-22, matches
...
│   │                            yes), then "Porch can't guess these" (ambiguous or unknown) full
...
│   │                            "Let Porch read this" switch, and delete.
...
│   ├── VoiceInputButton.tsx   — mic → `transcribe`; renders null w/o MediaRecorder. Takes `value`/`onChange` (not `onTranscribed`) since 2026-08-18: it owns the text after the anchor point and rewrites it live as words stream in, so a failure keeps what arrived. Live level meter from an AnalyserNode; 10-min cap.
...
│   │                            `onAsk` optional: no handler, no "Ask Porch" row (the demo).
...
| `transcribe` | Streaming speech-to-text (`gpt-transcribe`), SSE out. Auth-gated + roster keywords since 2026-08-18. |
...
notebooks.locked
              — 2026-08-19, bool default false. Enforced INDEPENDENTLY of ai_visible:
                ...
                left on — Porch reported no notebook entries at all.
...
30. AI/"fuzzy" semantic search — **resolved by routing, not by building, 2026-08-12 (see item 14).** Global search's `Ask Porch about "…" →` row hands anything word-matching can't answer to `converse`, which already reasons over the whole corpus. A dedicated semantic endpoint stays unbuilt on purpose: `converse` is a zero-tool full-dump design, so a tool-call round trip re-sends the whole ~30k-token prompt, and there is no embeddings/pgvector infrastructure in this project at all. Revisit only with evidence from real use.
...
102. ~~Rename the app from Boomer to Porch~~ — **DONE 2026-08-22.** Item 72 step 3, pulled forward because the App Store plan makes it blocking: the iOS **bundle ID is permanent once a build is uploaded**, so the name has to be settled before any Capacitor work starts. Founder chose Porch after a survey of what's actually available — every plain descriptive name in this category is taken, including **Kinship** (a personal CRM pitched almost exactly as this app is) and **Fondly: Memory Journal**. Apple doesn't require a unique word, only a unique name string, which is why live apps carry descriptors ("Porch: See Your Friends" already exists — a known, accepted collision). **Flagged to the founder and accepted:** Porch Group is a public company with a "Porch" trademark in home services; different category, so low risk, but not a hobby-project neighbour. Scope was 85 occurrences across 34 files, not the "four places" `PWA.md` claimed (that table is now corrected there, along with why the icon script had never worked on Windows). All 10 AI Edge Functions redeployed — the assistant's name for itself lives in their prompts. Icons and favicon regenerated with a "P" (the old favicon was still the default purple Vite mark). **Not renamed on purpose:** lowercase `boomer` storage keys, the `boomer-app-2-eight.vercel.app` hostname, and `PROJECT_HISTORY.md`'s dated entries.
...
- **Item 96 click-testing, 2026-08-21 — most of it done, three paths still unseen.** Verified against the founder's real account at merge time, each write reversed afterwards and the queue counts confirmed back at baseline (294 pending / 6 selected / 189 rejected / 176 accepted): one Home nudge reading "1,122 things to review"; all four inbox rows with counts matching the DB; Quick Add creating a real event and Undo deleting it again cleanly; Remind Me applying the saved 30-day default without a sheet, and its Undo; the gender pass saving through the dropdown, dropping the name off the can't-guess list, and the count falling 60 → 59; the contacts queue with its All / Already in Porch / New people filters; the detailed card showing a possible-duplicate warning. **Still unseen:** a batch of 10 ending with the summary panel, a collapsed **Accept** on the detailed card saving the right title/date/attendees (skipped on purpose — unlike Quick Add it has no one-tap Undo, so testing it means leaving or hand-deleting a real event), and **drag**-into-Men/Women (only the dropdown path was exercised; both call the same setter, but the drag handlers themselves have never run).
