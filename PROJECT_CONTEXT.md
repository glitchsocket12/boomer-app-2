...
61. ~~First-person "my" misattributed to the wrong person~~ — **DONE 2026-07-26.** Root-caused to `add-fact` (the profile-scoped quick-fact bar — matches the Ken Miller repro exactly: text typed directly on Ken's profile). Its prompt framed all captured text as "about" the profile person with no signal that a first-person pronoun means the app's signed-in user instead; added an explicit instruction (using the existing `is_self` lookup) telling it first-person text refers to the self person, never to rewrite "my X" as "&lt;profile name&gt;'s X," and to leave the pronoun as typed if unsure. `converse`/`update-moment` weren't touched — their existing `buildSelfInstruction` already resolves unqualified "my"/"our" for relationship capture; only `add-fact`'s plain-note path had the gap. Verified live on the real Ken Miller profile: a new first-person fact ("my scout troop leader") now saves unchanged instead of being rewritten to "Ken's," test note removed after.
