...
- ~~Sibling-linking fix needs redeploy~~ — **deployed and confirmed live 2026-07-20** (`add-fact`/`converse`/`update-group`/`update-moment`, via `npx supabase functions deploy` with a founder-provided token). Verified end-to-end with disposable test people (deleted after): reciprocal write-onto-subject on confirming a suggestion, and co-named-siblings auto-linking to each other, both confirmed working via the "same person" path (bare name matching an existing full-named profile) and the "new person" path (bare name, nobody on file yet) — including the fail-safe case where two co-siblings don't share the subject's last name, which correctly declines to link rather than guess wrong. Ale/Fede/Manuel Sucre test data was separately hand-patched live via the fact bar the same day.
