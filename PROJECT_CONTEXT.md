...
- **Voice input on every conversational text box** (built 2026-07-16): a mic button (`VoiceInputButton.tsx`) next to the input on Home's main chat, the Event-detail "add more detail" chat (`UpdateMomentChat.tsx`), and the person profile's one-line fact bar. Press to record, press again to stop; the recording is sent to the new `transcribe` Edge Function (OpenAI Whisper), and the transcribed text is dropped into the text box for the user to review/edit before pressing Send — it does not auto-send, so a misheard word can be fixed first. Deliberately NOT the free browser Web Speech API originally planned, because that API's speech-recognition half has no support at all in iPhone Safari, and the founder's stated end goal is an iPhone app — see Section 8 for the reasoning. **Not yet click-tested end-to-end**: doing so requires (a) being logged in, which wasn't possible in the assistant's session without entering a password (against standing safety rules), and (b) an `OPENAI_API_KEY` secret in Supabase, which does not exist yet — the founder needs to get an OpenAI API key and add it as a Supabase Edge Function secret themselves before voice input will actually work. Build passes and the button renders/loads without errors; the actual record-to-text round trip is unverified.
...
- **Voice input is built (see Section 6) but not yet functional in production** — the `transcribe` Edge Function needs an `OPENAI_API_KEY` secret added in Supabase before it will work at all. The founder needs to (1) create/access an OpenAI platform account and generate an API key, then (2) add it as a Supabase secret named `OPENAI_API_KEY` (Supabase dashboard → Edge Functions → Secrets, the same place `ANTHROPIC_API_KEY` lives) — the assistant cannot do this step, since entering API keys/credentials into anything is against its standing safety rules. Also not click-tested end-to-end in-browser (requires logging in, which the assistant can't do without entering a password) or on a real device microphone.
...
- **No in-app way to remove a person from a group, or a group from a moment**, once tagged (`GroupDetail.tsx` has no removal UI at all). A miss-tagged member currently has to be fixed manually via the Supabase Table Editor's `person_groups`/`moment_groups` tables (see Section 9's "two people sharing a first name" bug for a real example).
