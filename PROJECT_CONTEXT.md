...
- **Founder cleanup needed: two separate "Barbara Bach" profiles exist** — found 2026-07-20 during the sibling-parent backfill (see PROJECT_HISTORY §16). Not touched — merge via People search + merge-profile once confirmed which one should survive.
- **Siblings now inherit shared parents (2026-07-20, see PROJECT_HISTORY §16)** — fixed the bug where adding a sibling via the family tree "+" picker never copied an existing sibling's parents onto the new person. Frontend fix (`writeRelationship.ts`) is live via Vercel. The edge-function mirror (`_shared/relationships.ts`, used by the chat/fact-bar sibling path) is committed but **needs a Supabase Edge Function redeploy** — no token was available this session; same manual step as prior rounds (§13/§14/§15/§16 in history).
