...
- Person profile page: chronological notes, a "fact bar" to add a detail directly. Routes through AI classification (`add-fact` Edge Function) that files the text wherever it best fits: a last-name correction, a birthday/anniversary date (upserts into `reminders`), or — if it's neither — a plain note. This is a general classifier, not a fixed list of hard-coded phrases, so it's expected to handle new phrasings of the same three cases reasonably well, but it currently only recognizes those three structured types plus the note fallback (e.g. it won't infer group membership or anything else not modeled above).
