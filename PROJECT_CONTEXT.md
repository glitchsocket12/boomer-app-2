...
94. ~~The AI saw a different group name than the app~~ — **DONE AND DEPLOYED 2026-08-11** (`add-fact`, `converse`, `scan-calendar-sources`, `suggest-prompts`, `update-moment` — see §10 for the deploy, including the `--no-verify-jwt` incident it caused and resolved). `_shared/groupNames.ts` is the server twin of `lib/groupDisplayName.ts`. The app copy was changed 2026-08-03 to qualify through the WHOLE ancestor chain ("Squadron / Alpha Flight / Pilots") precisely because one level stops being unique past two levels of nesting; the server copy never got that change. Two "Pilots" under two different "Alpha Flight"s therefore produced the same key server-side and `idByQualified` kept whichever was indexed last — the exact wrong-subgroup TAGGING bug that file exists to close, reopened from the other side. `qualify()` now walks the full chain (same cycle guard, same missing-ancestor truncation as the app copy) and `splitParent()` matches the LONGEST existing prefix rather than the first separator, so "Squadron / Alpha Flight / New Thing" creates New Thing under Alpha Flight instead of a group literally named "Alpha Flight / New Thing". Found by writing `src/lib/groupNamesParity.test.ts`, which failed 4 of its 7 cases on the first run.
...
- **Not yet click-tested against real data (2026-08-11):** items 88, 93 and the `DueForUpdate` fix are verified by build + 371 tests + typecheck + a clean dev-server load, but not by a logged-in pass — the assistant's browser pane wasn't displaying, so screenshots and clicks weren't available this session. Worth one login to confirm: "Due for an update" no longer lists people written about recently, a big group's member count matches what the AI reports, and the People/Groups/Events/Calendar lists still render.
