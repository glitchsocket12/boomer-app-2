...
- **Founder action needed: turn on "Sign in with Google" (2026-08-03)** — code's done and click-tested (confirmed it correctly calls Supabase's `/auth/v1/authorize?provider=google`, currently returns `400 provider is not enabled`, exactly as expected pre-setup). Two steps left, both outside this repo, reusing the same Google Cloud OAuth client already set up for Google Photos rather than creating a new one: (1) **Google Cloud Console** → Credentials → open that existing OAuth Client → Authorized redirect URIs → add `https://dedtnytxhzzjimkozncc.supabase.co/auth/v1/callback` → Save. (2) **Supabase Dashboard** → Authentication → Providers → Google → enable → paste that same Client ID + Client Secret (the ones already used for `GOOGLE_PHOTOS_CLIENT_ID`/`SECRET`) → Save. Two things worth knowing, not action items: while that OAuth consent screen stays in Google's Testing mode, only accounts added as test users can complete Google sign-in either (same limitation as Google Photos, §2/§10) — basic sign-in scopes (email/profile) shouldn't need Google's full verification review to publish, unlike Photos' scope, but worth checking the consent screen's publish status if this should work for people beyond test users. And since public signup is closed (below), a brand-new Google account with no existing Boomer login should get turned away by Supabase's own signup-disabled setting rather than silently creating an account — worth confirming once the two steps above are done, by testing with the founder's own existing-email Google account first.
