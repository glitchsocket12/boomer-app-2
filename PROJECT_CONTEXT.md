...
│   │                            event/group/note only — DemoShell's Crumb has no pet or tag.
...
97. **Notebooks in the landing-page demo** — open. `src/pages/demo/DemoShell.tsx` has its own nav and static fixtures, so the public demo shows an app without the Notebooks tab while the real one has it. `Notebooks.tsx` already exports `NotebooksView` for exactly this, so it's fixtures plus a nav entry, not a rebuild. Same zero-Supabase/zero-API rule as the rest of the demo (CLAUDE.md rule 3).
...
- **Founder action needed: run `migrations_manual/2026-08-12-subgroup-color.sql` (item 82's manual-override half).** Adds nullable `groups.color_index`. Until it runs the app fails open — tiles keep their automatic colours and the recolour swatch stays hidden — so this is genuinely optional: run it only if the auto-assigned subgroup colours actually bother you, which was the founder's own condition for building this at all.
