...
- **Founder action needed: deploy `import-contacts` (contacts-import CPU-limit fix, 2026-07-26)** — `npx supabase functions deploy import-contacts --project-ref dedtnytxhzzjimkozncc` with a founder-provided access token (no token available this session). Until deployed, importing a large real contact list still fails with the "couldn't read that file" error (root cause is actually the matching code, not the file — see §7 Contacts import entry, bug fix #2).
