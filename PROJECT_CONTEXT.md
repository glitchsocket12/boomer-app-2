...
113. **Annual wraps as notebook entries** — founder, 2026-08-23: *"integrations with apps people use, that show their lives through interesting lenses with annual wrap ups… Spotify, Strava, etc."* **Both apps named are permanently closed to Grove, so this is never an OAuth job:** Strava's API Policy (June 1, 2026) forbids its data "in connection with the development, training, evaluation, or operation of any AI Application," naming "ingestion into a context window or working memory" — which is exactly what a notebook entry does in `converse` — and caps Standard Tier at 10 users behind a paid developer subscription; Spotify caps new Development Mode apps at 5 authenticated users and gates extended quota behind a registered business with ≥250k monthly active users, which §9's friends-and-family ambition will never meet. **The reframe the founder approved instead: the user already HAS their Wrapped and loses it by January** — nobody still has their 2019 one — so Grove keeps the artifact rather than recomputing it. That needs no API, no approval and no policy exposure, and it covers every provider at once, including ones with no API at all (Apple Music Replay, YouTube Recap, Duolingo, Peloton, Reddit Recap) and Strava's own Year in Sport, which Grove may not call but the user may freely screenshot. Founder's three decisions: (a) no provider APIs; (b) it lands as a **notebook entry that can point at real events** and people, not an event of its own — a wrap has no attendees and no day it happened on, so it would sit wrong in "Who was there" and put a fake date on the calendar; (c) capture is *"share the wrapped to the app and have it pull all the various data to it or to save it entirely"* — **both halves**, store the image AND read the numbers off it. **The share sheet is the right front door and is gated on item 105:** iOS Safari has never supported the Web Share Target API, so a PWA cannot appear in the iPhone share sheet at all — that needs a native Share Extension, which is what the Capacitor app buys; the Android equivalent needs a service worker, which PWA.md deliberately declined. Until then the wrap is uploaded from Photos, one extra tap. Reuses, if built: the private `photos` bucket + `createSignedUrls` (PhotoGallery.tsx), the browser→function base64 handoff (ContactsImport.tsx → `import-contacts`), `notebook_entry_people` as the exact model for a new `notebook_entry_moments`, SearchAddPicker + `momentDisplayName.ts` for the event picker, and `converse`'s existing tier-3 notebook block — so no new AI surface and no new tokens beyond the entry text. The one new call is a single **vision** read per wrap (`claude-sonnet-5` supports image blocks), cached to a column forever — CLAUDE.md rule 3's approved shape: ~half a cent, once, never re-read. Sequence with **106** ("Your Year" is a default notebook NAME, never an app-supplied category — §1 still holds) and **105**. Full policy findings and sources in PROJECT_HISTORY.md.
