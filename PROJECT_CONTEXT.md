...
- **Founder action needed (2026-08-23): run the group-active-window migration and redeploy `converse`.** `migrations_manual/2026-08-23-group-active-window.sql` adds `groups.start_date`/`end_date`/`location`; then `npx supabase functions deploy converse`. **NOT YET RUN** — this session had no Supabase token or `.env`. Until both land, every reader fails soft by design: GroupDetail hides its "When and where" row, `suggestEventGroups` falls back to the attendance rule alone, and `converse` renders the old roster line. The work is also on branch `claude/crm-school-event-tagging-xs6emu`, NOT `main` — it does not auto-deploy until that branch is merged.
