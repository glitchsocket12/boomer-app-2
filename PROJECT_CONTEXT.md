...
│                                **Copy all (2026-08-30):** the list panel's header copies every
│                                open note as one text block (`formatFeedbackNotesForExport`,
│                                grouped by page, oldest-first within a page, 9 tests) for pasting
│                                into a Claude Code session. `feedback_notes` is RLS-scoped to the
│                                founder's own login, so a session with no browser session of
│                                theirs — a cloud one especially — has no other route to these
│                                notes. Falls back to a select-it-yourself box when
│                                `navigator.clipboard` is missing or refuses (Safari, insecure
│                                context); both paths click-tested in Chromium.
```
...
**Items 114–116 came in via the click-to-comment feedback widget (§3 FeedbackWidget.tsx), founder session 2026-08-27, all three on the calendar triage screen ("Look through events" — `CalendarTriage.tsx`), folded in here instead of living only in the `feedback_notes` table. **Unlike the 2026-07-23 and 2026-07-25 batches these are still `open` in the widget** — the session that captured them lost its database connector before it could flip them, so clear them from the widget's own list (or hand that to a session that still has one):**

114. **Triage recognises a duplicate but never asks what to do about it** — founder: *"The app should be looking at dates/locations/times to recognize if an existing event or sub event is on file. Once it is recognized, it should let the person pick if it's the same event, and then after they pick, they should be asked if they want to add as a note, create a new sub event, or do it later."* Half of this already runs: `findLikelyMatch` (`lib/likelyDuplicate.ts`, free and client-side — title word-overlap corroborated by date proximity) prints *"Looks like "X", already on file."* and swaps Quick Add for a single **"Looks familiar — review it"** button into the detail flow. The pool is every row in `moments`, so sub-events are already matched against. What's missing is the whole fork after the match: the app **asserts** the duplicate instead of asking the person to confirm it, and there is no add-as-a-note / new-sub-event / not-now choice once they do. Two open questions for scoping: **location is displayed on the row but is not a match signal**, and `moments` store a date, not a clock time, so "times" as the founder wrote it has nothing to match on today. All three destinations exist (event notes, `parent_moment_id` sub-events, item 96's Remind Me), so this is a confirm step plus wiring, not new machinery.

115. **"Remind Me" stopped asking, and a month is too long** — founder: *"The app needs to ask when to remind them in the pop, 30 days is too long."* Working as built, which is the problem: `handleRemind` opens `RemindSheet` **only** when `user_settings.review_remind_days` is NULL, and defers silently on the saved number otherwise. So one tick of the sheet's own *"Use this as my default, don't ask again"* checkbox — presumably during the 2026-08-19 round-2 pass — froze every later Remind Me at 30 days, and nothing in Settings or on the triage screen can change or clear that column. The door has no handle on the inside. Fix: always offer the sheet with the saved choice preselected (or give the default a visible off switch), and add intervals shorter than today's 7/30/90/180 — `REMIND_OPTIONS` is frontend-only, no migration behind it.

116. **Turned-down candidates can only be undone, never deleted** — founder, on the "206 turned down — review/undo" row: *"Once clicked, should have a place to delete them permanently, not just undo. This will help cull the total amount of stuff to review."* **The trap to design around:** `scan-calendar-sources` builds its skip-list (`seenUids`) from every existing `moment_import_candidates` row *regardless of status*, so the rejected row is precisely what keeps that event from coming back — a plain DELETE means the next scan re-imports it and the pile regrows. A permanent delete therefore needs a tombstone (keep `ical_uid`, drop the payload) or a separate dismissed-uid table. Pairs with item 104's "archive the whole queue, recoverable" escape hatch, and with it covers both stated halves of "too much to triage."
