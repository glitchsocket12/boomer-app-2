...
│   │                            pets"), loadAllPets, createAndLinkPet, linkPet, unlinkPet,
│   │                            updatePet, plus pure formatPetLine/formatPetDates/isMemorial
│   │                            (tested). Living pets sort before memorials.
...
│   │                            Contact Info and Associated Groups. Same shape as
│   │                            ContactInfoSection (own isolated query, EditButton → draft →
│   │                            save, `readOnly` hides every control) but backed by real
│   │                            `pets`/`person_pets` tables, not columns. Renders nothing at all
│   │                            when the migration hasn't run — an Add box over a missing table
│   │                            would swallow the write silently. Add and attach are ONE gesture:
│   │                            the SearchAddPicker searches every pet on the account, so tagging
│   │                            the spouse's dog is the same motion as creating a new pet. The
│   │                            remove control UNLINKS (the pet may be someone else's too).
│   │                            Reuses ContactInfoSection's exported LabeledValueEditor for the
│   │                            free-form Details rows. Demo passes `pets={[]}` so the public
│   │                            demo still makes zero Supabase calls.
...
| `update-moment` | Event chat. Saves per turn (not on "done"), has full people+events rosters, `moment_field_updates` (when/where/title), `add_groups`, relationship signals, self-person "my X" resolution (2026-07-20). "Today's date" is time-zone-aware, same fix/mechanism as `converse` above (2026-07-24). **2026-07-30:** same roster-read parallelization, general/null-person notes, date sanitization, and expanded date-phrase examples as `converse` above — deployed live. |
...
person_pets   person_id + pet_id (PK), index on pet_id (reverse lookup: who owns
              ...
              person leaves an orphan pets row (known, accepted: the alternative
              risks destroying a shared household pet during a merge).
...
- **Pets** (item 73, 2026-08-01): profile UI + schema built, **awaiting the founder's migration run (§10) — the card is invisible until then, by design.** Pets are shared records (`pets`/`person_pets`), so the family dog lives on both spouses' profiles and is edited once; the picker searches every pet on the account, making "add a new pet" and "attach the spouse's dog" the same gesture, and removing unlinks rather than deletes. Species is free text and each pet carries an open `{label, value}` Details list, so a fish and a horse both fit without new columns. Deceased pets sort last, render as "In memory," and get no follow-up nudges. `converse` read/write, merge/delete handling, and the "a pet is not a person" guards in the sibling functions are still to come — see §8 item 73 for the hard-gated order.
...
73. **Pets on a profile** — requested 2026-08-01, **UI + schema DONE, chat wiring NOT done.** Founder decisions taken up front: a pet is its OWN record attachable to one or more people (household dog edited once, shows on both spouses' profiles), and the Home chat should eventually both read AND write pets. "Somewhat customizable to accommodate the variety of pets" → species is free text, plus an open `{label, value}` Details list per pet (Barn/Tank/Vet), not a fixed field set. Shipped: `pets`/`person_pets` tables (§6), `lib/pets.ts`, `components/PetsSection.tsx` (§3). **Still to do, in this order:** (B) founder runs `migrations_manual/2026-08-01-pets.sql` — see §10; (C) `converse` pet roster + write path — **must NOT be deployed before B**, or the write fails and the AI still cheerfully claims it saved (§12's headline bug); (D) `PersonDetail.tsx` merge/delete `person_pets` handling — also gated on B, and the delete half specifically because `handleDeleteProfile`'s `Promise.all` aborts on any error, so adding it pre-migration would break delete-profile outright; (E) a one-sentence "a pet is not a person" guard in `add-fact`/`update-moment`/`update-group` (today "Sarah got a puppy named Biscuit" most likely creates a *person* named Biscuit, in the People list and the Dunbar count). **Deliberately deferred:** a "Pets" Key Facts category in `person-facts` (would force an AI regeneration sweep across every profile for info the Pets card already shows), pet birthdays on the Calendar (needs `reminders.pet_id` + CHECK/RLS changes), demo pet content, a `PetChip`, a global Pets page.
...
- **Founder action needed: run `migrations_manual/2026-08-01-pets.sql` (item 73)** — creates `pets` + `person_pets`. Frontend is already live and deliberately renders NOTHING until this runs (PetsSection distinguishes "no pets" from "tables missing"), so there's no broken state in the meantime — the Pets card simply doesn't appear on any profile. Paste into the Supabase SQL Editor. **Steps C and D of item 73 are hard-gated on this** — do not deploy `converse`'s pet write path first.
