...
- **Needs deploy + backfill: family tree relationship-sync fix (2026-07-25, item 40 follow-up)** — code committed (`writeRelationship.ts`/`_shared/relationships.ts`/`RelationshipSuggestions.tsx`/`FamilyTree.tsx`, see §3), verified live with disposable test people, but two things still need a founder-provided Supabase access token (none available this session, same blocker as the row above): (1) redeploy `add-fact`/`converse`/`update-moment`/`update-group` (all four import the changed `_shared/relationships.ts`) so the fix applies to relationships captured via chat/fact-bar, not just the Tree/Circle "+" pickers (those already work now — the frontend deployed live on push); (2) run `migrations_manual/2026-07-25-spouse-coparent-backfill.sql` FIRST, then `2026-07-25-shared-parent-sibling-backfill.sql`, to fix already-built trees (the reported Lorenzo Harris tree, and everyone else's) — dry-run the spouse-coparent file's inner `select` first and eyeball it before running for real, specifically confirming Michael Galchinsky isn't in the result (see that file's own header comment for why).
