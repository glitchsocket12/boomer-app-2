...
│   │                            expands to indented child cards. Not yet migrated
│   │                            live — see §10.
...
│   │                            children become independent top-level events. Not
│   │                            yet migrated live — see §10.
...
- **Founder action needed: run `migrations_manual/2026-07-30-moment-sub-events.sql`** (item 35, sub-events) — adds the nullable `moments.parent_moment_id` column. No Supabase access token was available this session to apply it directly via the Management API. Code (EventDetail's "+ New Sub-event"/"Sub-events" section, Events.tsx's collapsible sub-event grouping) is deployed and confirmed failing open in browser preview — clicking "+ New Sub-event" today shows a clean "Couldn't create a sub-event" error instead of crashing, and the rest of the event page still works normally. Until this runs, sub-events can't actually be created.
