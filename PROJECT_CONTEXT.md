...
│   │                            in place rather than clearing a separate query box. Wired to
│   │                            ImportReview.tsx, ContactInfoSection.tsx (addresses, no recents)
│   │                            and — 2026-09-03, founder ask "we definitely need to have it on
│   │                            events too" — EventDetail.tsx's name/date/location edit form,
│   │                            whose recents are every other event's `location` tallied by
│   │                            `tallyLocations` (lib/locationGroups.ts, reused from
│   │                            ManageLocations) most-used first, since only the top 5 show.
│   │                            Optional `inputStyle` merges over the box's own styling —
│   │                            EventDetail passes `metaEditInput` so Location carries the same
│   │                            padding as the Date box beside it (`renameForm` is a wrapping
│   │                            flex row, alignItems center), and the label gets a 16rem minWidth
│   │                            because the component's box is width:100% and would otherwise
│   │                            collapse where the plain input had a browser default.
...
- **Address autocomplete on the event page (2026-09-03) — browser-verified in isolation; the live half is still unseen.** Founder ask after confirming Geoapify works elsewhere: "we definitely need to have it on events too." `EventDetail.tsx`'s Location field was a plain text box; it is now `AddressSuggestInput` (see §3). **Verified in headless Chromium against a throwaway harness that reproduced `renameForm` exactly** (the harness is not in the repo): the dropdown renders inside the `<label>` and paints above its siblings rather than being clipped, click-select and ↓+Enter both fill the field, a bare Enter still submits (saves) as before, Location's height matches the Date box, and at a 375px viewport the row wraps with `scrollWidth` 375 — no sideways scroll, the item-84 nav bug's failure mode. **What the harness could NOT cover:** this container has no `VITE_GEOAPIFY_API_KEY`, so only the local "previously typed" half was exercised — the live Geoapify half is the same dropdown code but has never been seen on this screen. Also unproven against real data: that `loadEventRoster()` (kicked off by `startEditingBasics`) actually populates recents on a real account. On a slow connection the local suggestions appear a beat after the live ones, by design.
...
- ~~**Item 113 (Associate an event), 2026-08-26 — needs one click-through on the real account.**~~ — **VERIFIED BY THE FOUNDER 2026-09-03: "Related events works."** Item 113 is closed. Original note kept for the record: `moment_links` is applied and its RLS proved by a rolled-back insert as role `authenticated`, and every UI branch was click-tested through the real `EventDetailPage` against a stubbed backend — but no logged-in session was available in the pane, so no association has yet been made against real data. Worth doing once: link two real events as related, and pull one existing event under another, then undo both.
...
- **"Sign in with Google" — 2026-09-03: the Supabase Site URL was still `http://localhost:3000`, founder has now changed it; needs one more end-to-end try.** The founder's 2026-09-03 attempt got all the way through Google's consent screen and Supabase issued a real session — the browser landed on `localhost:3000/#access_token=…` with `ERR_CONNECTION_REFUSED`, which is the sign-in *succeeding* and only the final redirect going to the wrong host. `Login.tsx` passes `redirectTo: window.location.origin`; Supabase only honours that if the origin is in its Redirect URLs allowlist and otherwise falls back to Site URL, which was never moved off the dev default. Founder updated the URL/redirect the same day — **worth one more sign-in to confirm it lands on the app.** Note for `NATIVE.md`: this same `window.location.origin` line is one of the two things that breaks in a Capacitor wrapper, for an unrelated reason. Earlier history: **Cloud Console + Supabase setup done 2026-08-03.** Founder completed both steps (redirect URI on the existing Google Photos OAuth Client; Google enabled as a Supabase provider with that Client ID/Secret) — first attempt 400'd with `redirect_uri_mismatch` (URI hadn't saved/was on the wrong client), retested after the founder fixed it and it now reaches Google's real sign-in screen (`accounts.google.com`, scoped to `email profile`, correct client_id/redirect_uri) with no error. Verified as far as possible without real Google credentials in this session — the founder still needs to actually complete a sign-in on the live site to confirm two things: (1) it lands them back in Grove logged in, and (2) since public signup is closed (below), it logs into their *existing* account by matching email rather than erroring or creating a new one. Also still true: while the OAuth consent screen stays in Testing mode, only accounts added as test users can get through this (same limitation as Google Photos) — basic sign-in scopes shouldn't need Google's full verification review to publish beyond that, but not yet checked.
