...
48. New Calendar feature — **phase 1 DONE 2026-07-24** (see §3 Calendar.tsx entry): nav tab with an upcoming list + real month grid, tag-filterable, auto-populated from existing moments/reminders, no mockup/placeholder needed since it reads real data directly. Verified live against the founder's real account (month nav, tag filter narrowing to zero on a tag with no July events, tile click → correct EventDetail, reminder click → correct PersonDetail). Month grid always renders a fixed 6-row/42-cell month (padded with muted adjacent-month days) so navigating between months of different lengths never shifts the page under a follow-up click — verified across 14 consecutive months, card position/height identical every time. **Phase 2 DONE 2026-07-24** (calendar-source connection): `calendar_sources` table (migration `2026-07-24-calendar-sources.sql`, applied via Management API), `validate-calendar-source` Edge Function (checks a pasted iCal URL server-side to dodge browser CORS — deployed), and a new `CalendarSettings.tsx` sub-page (reachable from both Settings and the Calendar page's own "Calendar settings →" link) to add/remove connected calendars by pasting their secret iCal URL — deliberately not full Google OAuth account access, no token storage needed. Verified live end-to-end against the founder's real account: added a real public ICS feed (validated server-side, saved, listed with "Not synced yet"), removed it cleanly; founder's own real entry ("Jake and Ceeb calendar," added independently by the founder testing the live site) was left untouched throughout. **Phase 3 DONE 2026-07-24** (import/suggestion pipeline): new `scan-calendar-sources` Edge Function — fetches each connected feed, parses VEVENTs with a new minimal shared parser (`supabase/functions/_shared/ics.ts`, RFC5545 line-unfolding + just the fields needed, not a full spec implementation), filters out cancelled/stale/solo-logistics entries, batches survivors (30/batch) through Claude with `cache_control`-tiered system prompt (stable instructions + the founder's `tags` list as suggestion-guidance, both cached 1h; batch data uncached — per CLAUDE.md's caching rules, same tiering shape as `update-moment`) to classify/extract occasion/location/when_text, matches ATTENDEE names against existing `people` in code (same idByName/nicknames algorithm as `update-moment`), and writes to a new `moment_import_candidates` table (migration `2026-07-24-moment-import-candidates.sql`) — never directly to `moments`, matching every other AI-suggestion flow in this app. Two trigger paths, founder chose "both" over picking one: a **manual "Sync now"** button on `CalendarSettings.tsx` (user's own JWT, scans just them) and an **automatic daily** `pg_cron` + `pg_net` job (`2026-07-24-calendar-sync-cron.sql`, 13:00 UTC, scans every connected account) authenticated via a shared secret stored in Supabase Vault (`cron_scan_secret`, never committed to git) matched against the `CRON_SCAN_SECRET` Edge Function secret. Verified live end-to-end against the founder's real connected calendar: pg_cron/pg_net enabled and job confirmed active (`select * from cron.job`), "Sync now" produced 11 correctly-extracted candidates in one run (e.g. "Wesley Gregorian's birthday," "85th Anniversary Dinner" with 2 matched attendees) — worth noting the founder's real calendar has several duplicate birthday entries under distinct UIDs (not a pipeline bug, confirmed by checking `ical_uid` values; the review UI is the natural place to reject the extras). Remaining from the gameplan (not yet built): the review-before-insert UI itself (`moment_import_candidates` rows exist but nothing lets the founder accept/reject them into real `moments` yet) and a Home-page nudge surfacing the pending count. Full design in plan file `i-want-to-gameplan-cuddly-wilkinson.md` (not checked into the repo).
