...
│   │                            Optional `inputStyle` merges over the box's own styling — the
│   │                            EventDetail row is a flex row aligned at flex-end, so it has to
│   │                            match its name/date siblings' padding or it reads as misaligned.
...
- **Address autocomplete on the event page (2026-09-03) — needs one look on a real event.** Founder ask after confirming Geoapify works elsewhere: "we definitely need to have it on events too." `EventDetail.tsx`'s Location field was a plain text box; it is now `AddressSuggestInput` (see §3). Build, `oxlint` and 711 tests are clean, but **no remote session has Supabase credentials** (the standing constraint in §8 item 72), so this has never been rendered against a real event. Worth checking three things once: the suggestion dropdown actually drops (it is absolutely positioned inside a `<label>` in a wrapping flex row — the one layout the component had not been used in before), the Location box lines up with Name and Date beside it rather than sitting taller, and previously-used places appear alongside the "via Geoapify" ones. The recents list is empty until `loadEventRoster()` returns, which `startEditingBasics` now kicks off — so on a slow connection the local suggestions may appear a beat after the live ones.
