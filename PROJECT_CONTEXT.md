...
│   │                            same "untag them there" tooltip. Added via a 🐾 action-bubble row
│   │                            that only renders when pets exist AND moment_pets is migrated;
│   │                            no create-a-pet path in there on purpose, since a pet needs an
│   │                            owner and owners are picked on a profile) +
...
│   │                            "Let Boomer read this" switch, and delete.
...
│   │                            `onAsk` optional: no handler, no "Ask Boomer" row (the demo).
...
notebooks.locked
              — 2026-08-19, bool default false. Enforced INDEPENDENTLY of ai_visible:
                ...
                left on — Boomer reported no notebook entries at all.
...
30. AI/"fuzzy" semantic search — **resolved by routing, not by building, 2026-08-12 (see item 14).** Global search's `Ask Boomer about "…" →` row hands anything word-matching can't answer to `converse`, which already reasons over the whole corpus. A dedicated semantic endpoint stays unbuilt on purpose: `converse` is a zero-tool full-dump design, so a tool-call round trip re-sends the whole ~30k-token prompt, and there is no embeddings/pgvector infrastructure in this project at all. Revisit only with evidence from real use.
...
- **HELD, NOT SHIPPED (2026-08-21, item 101): `converse`'s share of former names.** Founder's call — the file also holds an uncommitted pets-at-events edit from a different session that day (a `moment_pets` prompt bullet, its JSON field, and its apply loop), and deploying the former-name half would have carried that live unverified. Everything else in item 101 IS shipped. **Consequence while held: mentioning someone by their old name in chat still opens a second profile** — the one gap the item was meant to close. The changes are written and `deno check`-clean in the working tree, never committed. What `converse` needs, if it has to be redone: `former_last_names` in the roster select; a `formerById` map filled from `parseFormerLastNames` and kept OUT of `altNames`; `claimFormerNameKeys(people ?? [], idByName, claimKey)` before the `ambiguousKeys` sweep; `formerNameMarker` as a third entry in the roster `marks` array; `FORMER_NAME_ROSTER_CLAUSE` above the "also goes by" paragraph and `FORMER_NAME_PROMPT_CLAUSE` after the nickname clause; `FORMER_NAME_JSON_FIELD` after `nickname_updates` in the response shape (and in the parse-failure default); and a `former_name_updates` apply loop mirroring the nickname one. All six helpers already exist and are committed in `_shared/formerNames.ts`.
