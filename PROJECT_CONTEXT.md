...
│   │                            filter, and an "N turned down — review/undo" toggle. Keep writes
│   │                            'selected' (and NOT `reviewed_at` — keeping isn't reviewing); "Not
│   │                            this one" writes 'rejected', because that is what it is (reusing
│   │                            'skipped' would conflate the founder's no with the machine's — see
│   │                            §6). `Review N kept events →` hands off to ImportReview.tsx. This
...
│                                come back in a month" with Undo. Undo restores 'selected', not
│                                'pending' — the founder already triaged it and shouldn't be asked
│                                twice. Everything below still describes the expanded card.
...
│   │                            get the same treatment and need no rewrite here.
