...
- **Voice input on every conversational text box** (built 2026-07-16, UX fixed same day after founder testing): a mic button (`VoiceInputButton.tsx`) next to the input on Home's main chat, the Event-detail "add more detail" chat (`UpdateMomentChat.tsx`), and the person profile's one-line fact bar. Press to record; a speech bubble above the button shows a live "Listening… 0:0X — tap the mic again when you're done" state (with a pulsing dot) so it's unambiguous the app is recording and what to do next, then "Turning that into words…" while transcribing. The recording is sent to the `transcribe` Edge Function (OpenAI Whisper), and the transcribed text is dropped into the text box for the user to review/edit before pressing Send — it does not auto-send. Deliberately NOT the free browser Web Speech API originally planned, because that API's speech-recognition half has no support at all in iPhone Safari, and the founder's stated end goal is an iPhone app — see Section 8 for the reasoning.
  - **First real-world test (2026-07-16) surfaced two problems, both since addressed:** (1) the button gave no feedback about what state it was in or what to do next (just a color change) — fixed with the speech-bubble status described above; (2) transcription silently failed every time with no visible error. Diagnosed by directly calling the deployed `transcribe` function with a synthetic audio clip: the function *is* deployed and reachable, but the OpenAI call itself returned an error, almost certainly because the `OPENAI_API_KEY` Supabase secret is missing, mistyped, or the OpenAI account behind it has no billing method on file (Whisper's API rejects requests without one). Errors are now shown to the user as an actual on-screen message (e.g. "Couldn't turn that into text — the voice service may not be set up yet") instead of silently resetting, but **the underlying OpenAI key setup itself still needs the founder to verify/fix in the Supabase dashboard** — see Section 10.
  ...
  - **Still not click-tested fully end-to-end by the assistant**: doing so requires being logged in, which isn't possible in the assistant's session without entering a password (against standing safety rules).
...
- **Voice input is built (see Section 6) but still not functional in production as of 2026-07-16 testing** — the `transcribe` Edge Function is confirmed deployed and reachable (verified directly by the assistant), but the OpenAI transcription call it makes is failing every time. Most likely cause: the `OPENAI_API_KEY` secret in Supabase (Edge Functions → Secrets, same place `ANTHROPIC_API_KEY` lives) is missing, mistyped, or points to an OpenAI account with no billing method on file — **the founder needs to check/fix this in the Supabase dashboard**; the assistant cannot do this step itself, since entering API keys/credentials into anything is against its standing safety rules. Also still not click-tested end-to-end in-browser by the assistant (requires logging in, which it can't do without entering a password).
