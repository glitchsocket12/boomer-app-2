...
39. **Family tree layout engine rewrite — founder-proposed 2026-07-22, not yet built.** The incremental fixes in item 37 (bar stretches to reach the stem, wider `BRANCH_GAP`) are patches on a fundamentally backwards layout: today each tier (`layoutTier` in `FamilyTree.tsx`) is laid out independently from its own content, then centered independently on the canvas, and connector lines are stretched afterward to bridge whatever gap or overlap results. The founder's proposed fix flips this — descendants determine position, ancestors follow, same as a standard tree-layout algorithm:
    1. **Base tier = "You" (root-gen)**, laid out naturally as today — the one tier the page is always centered on.
    2. **Parents tier repositioned relative to it**: instead of laying out each couple/aunt-uncle-union independently, compute the midpoint of *its own children's* span in the base tier first, then place that union's marriage line centered exactly on that point (root's parents centered over root+siblings; each aunt/uncle sub-union centered over its own cousins).
    3. **Collision pass**: after centering every unit in a tier this way, walk left-to-right resolving overlaps — if two repositioned units' natural content width is wider than the gap between their target centers, push them apart symmetrically to a minimum clearance. This is what actually prevents the "looks connected but isn't" ambiguity (item 37's third follow-up), by construction instead of by guessing at a spacing constant.
    4. **Grandparents tier, same treatment one level up** — centered over its own children's span within the now-final (post-collision) Parents tier positions.
    5. **Kids tier stays as today's approach** (natural layout, stem stretches up to reach it) — it's the leaf end with nothing below it to anchor to.
    Bigger in scope than a constant tweak — a real rewrite of `layoutTier` plus new position-and-resolve passes in `src/pages/FamilyTree.tsx`. Founder asked for this to be picked up in a fresh session rather than continuing the (long) session that found items 37/38.
