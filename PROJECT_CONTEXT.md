...
│   ├── UpdateMomentChat.tsx   — event "add detail" chat → `update-moment`; renders both
│   │                            RelationshipSuggestions and MentionedPeopleSuggestions banners
│   │                            (the latter with `onApplied={onSaved}` so a confirmed profile
│   │                            shows under "Who was there" without a manual refresh)
