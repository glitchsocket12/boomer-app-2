...
│   ├── factRetraction.ts     — (2026-08-26) finds every note asserting one Key Fact, on BOTH
│   │                            profiles, plus the relationships row behind it. Pure + tested, no
│   │                            AI, deliberately over-inclusive — the user confirms what goes.
│   │                            Feeds components/RetractFactPanel.tsx ("That's not right").
...
│   ├── RetractFactPanel.tsx   — (2026-08-26) "That's not right" on a linked Key Fact: lists every
│   │                            note asserting it on BOTH profiles plus the relationships row,
│   │                            pre-ticked, and clears what's confirmed. Founder chose
│   │                            show-then-confirm over letting chat delete outright. Hidden in
│   │                            the demo (readOnly) and on facts naming nobody.
...
**Shared module** `_shared/howYouKnow.ts` (2026-08-26): the roster marker, the two prompt clauses (roster explanation + create rule, both written as wrong/right pairs) and `parseNewPersonEntry`, which accepts `new_people`/`add_people` entries as either a bare string or `{name, how_you_know_them}`. Used by `converse`/`update-moment`/`update-group` so the collision rule is identical at all three, and `how_you_know_updates` writes are additive-only (the profile form owns the field; chat only fills a blank). One deliberate prompt-cache invalidation on deploy — the clauses sit in each function's `stableInstructions`, never interpolated. 8 tests.
...
people        id, user_id, name (first), last_name?, nicknames? (comma-separated
              ...
              see §12), how_you_know_them? (2026-08-26, one free-text line placing
              someone their name alone can't — "Manuel's friend", "barista at
              Rosetta". The ONE alias-adjacent field that never feeds name
              resolution: display + AI-roster context only, or it would match the
              person it names. Searchable in People. See §12), key_facts jsonb?,
...
- **A colliding bare first name is never created blind, and never silently dropped** (2026-08-26). 561 of the founder's 896 people share a first name with someone else, across 149 colliding names (16 Alex, 15 David, 9 Sarah, 7 Julia). Every chat function unmaps such a name from its name→id index (`ambiguousKeys`) so it can't resolve to the wrong person — correct, and it caused two silent failures downstream for a year: a note naming "Sarah" was **dropped outright** (real content lost, reply confirmed it anyway), and "add Sarah" read as "no Sarah exists" and **minted another one** (how the account holds both "Amber" and "Amber h", and 26 duplicate name pairs). Both fixed: an unresolvable note now lands on the EVENT prefixed with the name as said, and a create is refused unless `how_you_know_them` comes with it. `_shared/howYouKnow.ts` holds the shared clauses so the fix stays in all three chat functions at once (the item-76 bug class).
- **A fact does not live in one row, so deleting one note does not retract it** (2026-08-26). Gus Reynolds is the worked example: two notes captured nine days apart in different words ("Is dating a girl named Olivia" / "Has a girlfriend named Olivia."), so deleting either leaves `people.key_facts` to regenerate the identical chip. Add the mirror note on the other person and the `relationships` row — which survives note deletion entirely and re-injects into Key Facts — and one wrong sentence hides in four places. **No AI path can edit or delete a note; every AI write is an insert**, so a correction spoken into chat appends a third sentence rather than removing the first two. The "That's not right" control on a linked Key Fact gathers all of it and clears what the user confirms (`src/lib/factRetraction.ts`, `components/RetractFactPanel.tsx`).
