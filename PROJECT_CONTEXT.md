...
candidates    id, user_id, calendar_source_id, ical_uid (unique per user, dedupes
              ...
              — 2026-08-12; 'skipped' means the AI scan judged it not worth
              suggesting and is recorded ONLY so it is never re-sent to the API,
              'rejected' stays the founder's own no. Every reader filters to
              'pending', so skipped rows are invisible to the UI), occasion?,
...
- **Second calendar-sync bug, found and fixed 2026-08-12 — independent of the API-key outage above, and it will still bite once the key is replaced.** `scan-calendar-sources` only ever wrote a row for an event the AI said WAS worth suggesting; an `include: false` was thrown away, so that event stayed out of `seenUids` and was re-sent to the API on every single run, forever. Measured on the founder's account: **322 events (140 + 182) re-judged every run, against a 240-event/8-batch cap** — so "Jake Personal" never reached the end of its own list, never got `last_synced_at` stamped, and read "Last synced Aug 4" for 9 days. Fix is three parts: a `'skipped'` status recording the AI's "no" (migration `2026-08-12-calendar-skipped-status.sql`, **already applied**), sources now processed **least-recently-synced first** so one calendar can't spend the whole batch budget every run, and the skip rows written in their own statement so a missing migration can never reject the genuine suggestions alongside them. Deployed (v25). **The ordering half is verified live** — "Jake Personal" went first and its timestamp moved Aug 4 → Aug 12. **The skip-recording half is NOT yet verified end-to-end**: with the key dead the API returns nothing to record, so zero skip rows have been written. Row shape was proved separately against the real table (insert + rollback). Re-check after the key is replaced: `select count(*) from moment_import_candidates where status='skipped'` should be in the hundreds after one sync, and both calendars' `last_synced_at` should advance on the same click.
