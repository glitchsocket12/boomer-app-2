...
48. New Calendar feature — **phase 1 DONE 2026-07-24** (see §3 Calendar.tsx entry): nav tab with an upcoming list + real month grid, tag-filterable, auto-populated from existing moments/reminders, no mockup/placeholder needed since it reads real data directly. Verified live against the founder's real account (month nav, tag filter narrowing to zero on a tag with no July events, tile click → correct EventDetail, reminder click → correct PersonDetail). Month grid always renders a fixed 6-row/42-cell month (padded with muted adjacent-month days) so navigating between months of different lengths never shifts the page under a follow-up click — verified across 14 consecutive months, card position/height identical every time. **Phase 2 code-complete 2026-07-24, not yet deployed** (see §10): `calendar_sources` table (migration `2026-07-24-calendar-sources.sql`, not yet applied), `validate-calendar-source` Edge Function (checks a pasted iCal URL server-side to dodge browser CORS — not yet deployed), and a new `CalendarSettings.tsx` sub-page (reachable from both Settings and the Calendar page's own "Calendar settings →" link) to add/remove connected calendars by pasting their secret iCal URL — deliberately not full Google OAuth account access, no token storage needed. Verified live that both missing pieces fail gracefully client-side (empty-state message when the table doesn't exist, clean error text when the Edge Function call fails) rather than crashing the page. Remaining phases from the gameplan (not yet built): the AI import/suggestion pipeline reading connected feeds with a founder-editable tag list (reusing the existing `tags` system, not a hardcoded enum) guiding what counts as suggestion-worthy, a review-before-insert UI, and a Home-page nudge surfacing pending suggestions. Full design in plan file `i-want-to-gameplan-cuddly-wilkinson.md` (not checked into the repo).
...
- **Calendar feature phase 2 needs deploying** (item 48): `calendar_sources` migration not yet applied, `validate-calendar-source` Edge Function not yet deployed — needs a fresh Supabase Personal Access Token from the founder to apply via the Management API / `npx supabase functions deploy`, or the founder pastes both into the Supabase dashboard directly.
