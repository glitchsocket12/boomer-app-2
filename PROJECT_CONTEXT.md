...
- ~~**RLS unverified on the pre-migration tables**~~ — **VERIFIED LIVE 2026-08-01, all clean.** Founder ran `migrations_manual/2026-08-01-rls-audit.sql`: all 23 tables (incl. `storage.objects`) have RLS on, every read policy scoped to `auth.uid()` or an ownership check through the parent table, nothing evaluating to a bare `true`. **Why the undocumented dashboard-made tables were covered anyway: `rls_auto_enable`**, an event trigger from the original Bolt/StackBlitz scaffold that auto-enables RLS on every `CREATE TABLE` in `public` — correctly written (`SECURITY DEFINER` with a pinned `search_path`). **Leave it in place.** §6's "RLS on everything" is now evidence, not a claim. Two low-severity loose ends remain, see `SECURITY.md` gap 0: `group_associations` only ownership-checks `group_id_a` (allows junk rows, leaks nothing), and two INSERT policies (`home_suggestions`, `notes_group_insert`) have no migration file so their WITH CHECK is still unverified — section 3 of the audit script now prints these.
